...
if (insertIndex !== -1 && !content.includes('DashboardHome')) {
  content = content.slice(0, insertIndex) + componentsToInject + '\n' + content.slice(insertIndex);
}

// 3. Replace the main layout in App return (keeping the inner flex-1 content intact)
const oldReturnRegex = /<div className="min-h-screen bg-transparent text-\[\#e1e1e6\] selection:bg-\[\#2DD4BF\]\/30 flex flex-col relative">([\s\S]*?)<div className="flex-1">/;
...
// Replace ExploreView with DashboardHome
content = content.replace(
  /{currentPage === 'explorar' && <ExploreView[^>]+>}/g, 
  "{currentPage === 'explorar' && <DashboardHome setCurrentPage={setCurrentPage} t={t} />}"
);

// Fix the closing tags at lines ~1110-1112
...
// We need to change the first `</div>` to `</div></main>` or similar.
// Since we opened `<main className="...">` and `<div className="flex-1 px-4...">`,
// The original file had `<div className="min-h-screen...">` and `<div className="flex-1">`.
// We replaced both tags with `<div className="flex h-screen...">`, `<main className="...">`, and `<div className="flex-1...">`
// Wait, we replaced TWO opening tags with THREE opening tags!
// So we need to add ONE MORE closing tag.
const oldClosingTags = `        </div>
...
content = content.replace(oldClosingTags, newClosingTags);

fs.writeFileSync(filePath, content, 'utf-8');
console.log('App.tsx updated SAFELY with ENNVO Dashboard Layout!');
