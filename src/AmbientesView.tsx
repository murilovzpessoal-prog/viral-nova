...
navigator.clipboard.writeText(template.promptText);
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Simulação do envio do prompt para a API
      console.log("Enviando prompt para a API: ", selectedTemplate.promptText);
      const response = await fetch('/api/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prompt: selectedTemplate.promptText })
      });
      
      if (!response.ok) throw new Error("API não disponível no momento");
      
      const data = await response.json();
      setResultImg(data.imageUrl);
      setIsGenerating(false);
    } catch (error) {
      console.warn("Usando imagem simulada por falha na API ou falta de backend.", error);
      setTimeout(() => {
              ...
              {isGenerating && (
                <div className="flex flex-col items-center gap-6">
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-[#8B5CF6] text-sm animate-pulse">Enviando prompt para a API e gerando...</p>
                  </div>
                </div>
              )}
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-6 mb-8">
        <div>
          <h1 className="text-3xl md:text-4xl font-light text-white mb-2 leading-tight">
            Navegue pelos <span className="font-medium bg-gradient-to-r from-emerald-400 to-[#10B981] text-transparent bg-clip-text">Cenários Premium</span> para o seu avatar
          </h1>
          ...
          <div key={template.id} className="relative group aspect-[4/5] rounded-[24px] overflow-hidden bg-white/5 border border-white/5 flex flex-col justify-end p-5">
            ...
            <div className="relative z-10 flex flex-col gap-2">
              ...
              <a
                ...
                download
                target="_blank"
                className="mt-2 flex items-center justify-center gap-2 w-full py-3 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 backdrop-blur-md border border-white/10 rounded-xl text-white text-[11px] font-bold uppercase tracking-wider transition-colors shadow-lg group-hover:shadow-[0_0_20px_rgba(16,185,129,0.3)]"
              >
                <Download className="w-4 h-4" />
                Baixar Cenário
              </a>
